<div class="ul-service-details-sidebar">
    <div class="ul-service-details-sidebar">
        <div class="ul-service-details-sidebar-widget">
            <span class="ul-service-details-sidebar-widget-title">Our Services</span>
            <ul class="ul-service-details-sidebar-links">
                <li><a href="invisalignaligners_clearaligners_treatment_in_bengaluru.php">Clear aligners / Invisalign <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="basal_implants_treatment_in_bengaluru.php">Basal implants / Immediate loading implants <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="regular_implants_treatment_in_bengaluru.php" class="active">Regular implants <i class="flaticon-arrow-up-right"></i></a></li>

                <li><a href="full_mouth_rehabilitation_treatment_in_bengaluru.php" class="active"> Full Mouth Rehabilitation (FMR)<i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="veneers_treatment_in_bengaluru.php" class="active">Veneers <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="smile_correction_treatment_in_bengaluru.php" class="active">Smile correction <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="digital_dentistry_treatment_in_bengaluru.php" class="active">Digital Dentistry  <i class="flaticon-arrow-up-right"></i></a></li>


                <li><a href="root_canal_treatment_in_bengaluru.php" class="active">Root canal Treatment <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="teeth_cleaning_treatment_in_bengaluru.php">Teeth Cleaning <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="dentalcrown_bridge_treatment_in_bengaluru..php">Crowns & Bridges <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="teeth_removal_treatment_in_bengaluru.php">Painless Teeth Removal <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="tooth_colored_fillings_treatment_in_bengaluru.php">Tooth Colored Fillings <i class="flaticon-arrow-up-right"></i></a></li>

                <li><a href="teeth_whitening_treatment_in_bengaluru.php">Teeth Whitening <i class="flaticon-arrow-up-right"></i></a></li>
                <li><a href="laser_dentistry_treatment_in_bengaluru.php">Laser Dentistry <i class="flaticon-arrow-up-right"></i></a></li>

                <li><a href="jaw_fractures_treatment_in_bengaluru.php">Fixing Jaw Fracture <i class="flaticon-arrow-up-right"></i></a></li>


        </div>
        </ul>
    </div>


    <!-- call to action card -->

</div>